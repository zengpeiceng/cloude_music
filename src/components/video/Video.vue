  ...
  <div id="video">
    <div class="center">
      <div style="margin-bottom: 10px;line-height: 30px">
        <h2>{{ details.title }}</h2>
        <span v-if="details.creator">by
          <router-link style="color: #0c73c2" :to="`/user/home?uid=${details.creator.userId}`"> {{ details.creator.nickname}}
          </router-link>
        </span>
      </div>
      <div class="v-content">
        ...
        <ul>
          ...
          <li><a href=""><i class="icon subscribeCount"></i><span>({{details.subscribeCount}})</span></a></li>
        ...
        <div class="sc-r-nav">
          ...
          <ul class="similar">
            <li v-for="(item, index) in related">
              <div class="cover">
                <router-link :to="`/video?id=${item.vid}`"><img :src="item.coverUrl" alt=""></router-link>
                <i>
...
import SongComment from 'views/song/SongComment'
  import { 
    getVideoUrl, getVideoDetails, getVideoOthersInfo, getRealtedVideo
...
export default {
  ...
  watch: {
    '$route.query': function(to) {
      this.getVideoMsg(to.id)
    }
  },
  created() {
    const vid = this.$route.query.id
    this.getVideoMsg(vid);
  ...
  methods: {
    getVideoMsg(vid) {
      ...
      getVideoOthersInfo(vid).then(res => {
        this.commentCount = res.commentCount;
